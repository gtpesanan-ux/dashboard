...
(() => {
  ...
  function initTree(root) {
    ...
    const movePill = (pill, target, visible = true) => {
      ...
      const scale = nr.width / Math.max(1, nav.offsetWidth);
      ...
      pill.style.width = `${tr.width / scale}px`;
      pill.style.height = `${tr.height / scale}px`;
      pill.style.transform = `translate3d(${(tr.left - nr.left) / scale - originX}px,${(tr.top - nr.top) / scale - originY}px,0)`;
    ...
    const updateLines = (animateActive = false) => {
      ...
      svg.style.opacity = '1';
      // Measure in SVG CSS pixels so zoomed/embedded layouts keep the same geometry.
      const nr = nav.getBoundingClientRect();
      const scale = nr.width / Math.max(1, nav.offsetWidth);
      const width = Math.max(1, nav.clientWidth);
      const height = Math.max(1, nav.scrollHeight);
      ...
      const centerY = (el) => {
        ...
        return (rect.top - sr.top + rect.height / 2) / scale;
      };
      const isVisible = (el) => el && el.offsetParent !== null
        && !el.closest('.dv-tree-collapse[aria-hidden="true"]');
      const listData = [...nav.querySelectorAll('.dv-tree-list')].map((list) => {
        const rows = [...list.children].map((child) => child.matches('.dv-tree-item')
          ? child : child.querySelector(':scope > .dv-tree-folder-toggle')).filter(isVisible);
        if (!rows.length) return null;
        // The branch finishes in the gutter, four pixels before the moving pill.
        const stop = Math.min(...rows.map((row) => (row.getBoundingClientRect().left - sr.left) / scale)) - 4;
        const spine = Math.max(4, stop - 12);
        return {list, rows, stop, spine, first:centerY(rows[0])};
      }).filter(Boolean);
      const branch = (entry, row, includeStem) => {
        const y = centerY(row);
        const radius = Math.min(5, (entry.stop - entry.spine) / 2);
        const start = includeStem ? Math.max(0, entry.first - 8) : Math.max(0, y - radius);
        return `M${entry.spine} ${start} V${y - radius} Q${entry.spine} ${y} ${entry.spine + radius} ${y} H${entry.stop}`;
      };
      const base = listData.map((entry) => entry.rows.map((row, index) =>
        branch(entry, row, index === entry.rows.length - 1)).join(' ')).join(' ');
      basePath.setAttribute('d', base);

      const selectedEntry = listData.find((entry) => entry.rows.includes(selected));
      if (selected && selectedEntry) {
        const activeBranches = [];
        let row = selected;
        let entry = selectedEntry;
        while (entry && row) {
          activeBranches.unshift(branch(entry, row, true));
          const folder = entry.list.closest('.dv-tree-folder');
          row = folder?.querySelector(':scope > .dv-tree-folder-toggle');
          entry = row ? listData.find((candidate) => candidate.rows.includes(row)) : null;
        }
        const active = activeBranches.join(' ');
