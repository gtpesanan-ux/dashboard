...
(() => {
  ...
  if (html.dataset.newsDetailRuntime === "v26") return;

  const q = selector => document.querySelector(selector);
  const main = q(".news-detail-main");
  if (!main) return;

  html.dataset.newsDetailRuntime = "v26";

  const PUBLIC_BASE = new URL("./", window.location.href).href;
  ...
  const showUnavailable = message => {
    document.title = "Artikel tidak ditemukan | PT DIRAC INOVASI NUSANTARA";
  ...
  setText("[data-detail-category]", text(item.category) || "Wawasan");
  ...
  if (sourceMeta) sourceMeta.textContent = "PT DIRAC INOVASI NUSANTARA · Panduan & Wawasan";
  ...
  if (takeaways.length) {
    const takeawayBlock = createContentBlock("Daftar periksa");
  ...
  if (integrity) {
    integrity.textContent = "Lanjutkan ke bacaan terkait untuk memahami topik yang berhubungan dengan kebutuhan Anda.";
  }
  ...
  if (newer || older) {
    ...
    if (newer) {
      ...
      link.textContent = "← Artikel sebelumnya";
      link.setAttribute("aria-label", `Artikel sebelumnya: ${text(newer.title)}`);
      nav.append(link);
    }
    if (older) {
      const link = document.createElement("a");
      ...
      link.textContent = "Artikel berikutnya →";
      link.setAttribute("aria-label", `Artikel berikutnya: ${text(older.title)}`);
      nav.append(link);
    }
    main.append(nav);
  }

  document.title = `${text(item.title)} | Wawasan PT DIRAC INOVASI NUSANTARA`;
